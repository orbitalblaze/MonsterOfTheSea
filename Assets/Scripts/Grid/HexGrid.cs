...
public class HexGrid : MonoBehaviour {
    ...
    public void TouchCell (Vector3 position) {
        ...
        int index = coordinates.X * height + coordinates.Z + coordinates.Z / 2;
        HexCell cell = cells[index];
        cell.color = touchedColor;
        hexMesh.Triangulate(cells);
        print("position"+position.ToString()+coordinates.ToString()+" Index = "+index);
    }
    ...
    void CreateCell (int x, int y, int i) {
        ...
        //il y ait un décalage de la moitié du double du rayon interne (soit le rayon interne)
        position.y = (y + x * 0.5f - x / 2) * (HexMetrics.innerRadius * 2f) /*Décalage entre les centres pour les colonnes*/;
        position.x = x * (HexMetrics.outerRadius * 1.5f) /*Décalage entre les centres pour les lignes*/;
        position.z = 0f;
